...
namespace HLTStudio
{
	public class BatchConv
	{
		...
		public void Perform()
		{
			...
			{
				string cuiDir = Path.Combine(BootTools.SelfDir, @"..\CUI");
				cuiDir = Path.GetFullPath(cuiDir);
				...
				foreach (List<Word> words in _wdTable)
				{
					if (Gnd.I.SndInputBatLocal)
						...
						writer.Write("\"" + StringTools.Combine(cuiDir, "SndInput.exe") + "\"");
		...
		private int FindT(int tMillis, int startPos, Word target) // //// // // /// /////
		{
			if (tMillis <= 0) // ///////
				return -1;
		...
		private void InsertMarginBetweenMCMB()
		{
			for (int index = 1; index < _words.Count; index++)
			{
				...
				{
					_words.Insert(index, new Word("T", 5)); // ////// //////
					index++;
				}
			}
		}
